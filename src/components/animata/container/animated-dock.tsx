  ...
  return (
    <div>
      <motion.div
        ...
        className={cn(
          "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-white/10 px-4 md:px-10 pb-3 dark:bg-blue-100/15 md:flex", // Large dock styles
          className,
          "backdrop-blur-sm",
        )}
      >
  ...
  return (
    <Link href={href}>
      <motion.div
        ...
        className="relative flex aspect-square items-center justify-center rounded-full bg-white/20 text-black shadow-lg backdrop-blur-md dark:bg-purple/15 dark:text-white"
      >
        <AnimatePresence>
          ...
          {isHovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit -translate-x-1/2 whitespace-pre rounded-md border border-gray-200 bg-white/80 px-2 py-0.5 text-xs text-neutral-700 dark:border-neutral-400 dark:bg-neutral-800 dark:text-white"
            >
              {title} {/* Tooltip text */}
            </motion.div>
          )}
    ...
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        ...
        {isOpen && (
          <motion.div
            layoutId="nav"
            className="absolute right-0 top-full mt-2 flex flex-col gap-2 p-4 rounded-lg bg-white/10 backdrop-blur-md border border-white/20 shadow-lg"
            initial={{ opacity: 0, x: 20 }} // Slide in from right
            animate={{ opacity: 1, x: 0 }} // Slide to position
            exit={{ opacity: 0, x: 20 }} // Slide out to right
          >
            {items.map((item, index) => (
              <motion.div
                key={item.title}
                initial={{ opacity: 0, x: 20 }} // Initial animation state - slide from right
                animate={{ opacity: 1, x: 0 }} // Animation to visible state
                exit={{
                  opacity: 0,
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="flex h-10 w-10 items-center justify-center rounded-full bg-white/20 text-black shadow-md backdrop-blur-md dark:bg-black/20 dark:text-white hover:bg-white/30 dark:hover:bg-black/30 transition-colors"
                >
      ...
      <button
        ...
        className="flex h-12 w-12 items-center justify-center rounded-full bg-white/20 text-black shadow-md backdrop-blur-md dark:bg-black/20 dark:text-white hover:bg-white/30 dark:hover:bg-black/30 transition-colors shadow-black/20 dark:shadow-white/20 shadow-sm"
      >
        {/* Render the appropriate icon based on open/close state */}
        {isOpen ? (
          <X className="h-6 w-6" /> // Show close icon when open
        ) : (
